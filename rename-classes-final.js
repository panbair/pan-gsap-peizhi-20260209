...
function processFile(filePath, suffix, componentName) {
  ...
  // 3. 修复插值表达式中的错误修改 (将属性访问中的 -数字后缀移除)
  content = content.replace(/\{\{([^}]+)\}\}/g, (match, expr) => {
    // 移除变量名中的 -数字后缀,但保留class名中的后缀
    const fixedExpr = expr.replace(/\b([a-zA-Z_$][a-zA-Z0-9_$]*?)-\d+\b/g, (match, varName) => {
      // 如果这个看起来像属性访问的一部分,移除后缀
      if (match.includes('.') || match.includes('[')) return match;
      return varName;
    });
    if (fixedExpr !== expr) changes++;
    return `{{${fixedExpr}}}`;
  });

  // 4. 修复 v-for 和其他属性中的错误
  content = content.replace(/:style="([^"]+)"/g, (match, styleContent) => {
    // 移除属性访问中的 -数字后缀
    const fixedStyle = styleContent.replace(/\b([a-zA-Z_$][a-zA-Z0-9_$]*?)-\d+\b/g, '$1');
    if (fixedStyle !== styleContent) changes++;
  ...
  // 6. 修改GSAP选择器 (querySelector/querySelectorAll)
