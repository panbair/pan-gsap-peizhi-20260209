<template>
  <div class="mg-morph-grid">
    <div class="mg-container">
      <h2 class="mg-section-title">变形网格</h2>

      <div class="mg-grid-layout">
        <div
          class="mg-grid-cell"
          v-for="(img, index) in images"
          :key="index"
          :class="{ expanded: expandedCell === index }"
          ref="items"
          @click="expandCell(index)"
        >
          <div class="mg-cell-inner">
            <img :src="img" :alt="`Image ${index + 1}`" />
            <div class="mg-cell-content">
              <span class="mg-cell-number">{{ index + 1 }}</span>
              <h3 class="mg-cell-title">{{ titles[index] }}</h3>
              <p class="mg-cell-desc">{{ descs[index] }}</p>
            </div>
          </div>
        </div>
      </div>
...
onMounted(() => {
  ctx = gsap.context(() => {
    // 标题动画
    gsap.from('.mg-section-title', {
      scrollTrigger: {
        trigger: '.mg-section-title',
        start: 'top 90%'
      },
      y: 50,
      opacity: 0,
    ...
    // 网格入场
    gsap.from('.mg-grid-layout', {
      scrollTrigger: {
        trigger: '.mg-grid-layout',
        start: 'top 85%'
      },
      scale: 0.8,
      opacity: 0,
    ...
    items.value.forEach((item, index) => {
      const inner = item.querySelector('.mg-cell-inner') as HTMLElement
      const img = item.querySelector('img') as HTMLElement
      const content = item.querySelector('.mg-cell-content') as HTMLElement
      const number = item.querySelector('.mg-cell-number') as HTMLElement
...
img {
  ...
  .mg-grid-cell.expanded & {
    height: 60%;
  }
}

.mg-grid-cell:hover img {
